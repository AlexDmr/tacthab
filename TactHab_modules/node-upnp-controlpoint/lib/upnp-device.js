 ...
 */
var UpnpDevice = function(controlPoint, uuid, location, desc, localAddress, rawData) {
	EventEmitter.call(this);
	// console.log("New device with IP", localAddress);
	...
	var req = http.request(options, function(res) {
		...
		res.on('end', function () { 
			if (res.statusCode !== 200) {
			  // XXX callback(new Error("Problem with getting basic event service desc " + service.serviceId), buf);
			}
			else {
