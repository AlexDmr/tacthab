	...
	var req = http.request(options, function(res) {
		...
		res.on('end', function () {
			if (res.statusCode !== 200) {
			  ...
			  console.error( "Problem with subscription on " + service.serviceId + " status HTTP " + res.statusCode, buf);
			  console.error( "Retry in 1 second" );
			  setTimeout( function() {self.subscribe(callback);}
						, 1000 );
			}
			else {
				if (TRACE && DETAIL) {
